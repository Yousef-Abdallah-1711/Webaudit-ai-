...
| 13 | Capability loader is a static import table, not filesystem-driven (T119–125) | **Made, not settled — and, per a final production-readiness review (2026-09-04), a real tension with CLAUDE.md's own non-negotiable #1 ("core never names a capability"), not merely a discovery-mechanism detail.** `apps/worker/src/orchestrator/capability-loader.ts` hardcodes six `import()`s rather than reusing `apps/api`'s `discoverCapabilities` (would cross the api/worker production boundary), and `apps/worker/src/reverify/resolve-check.ts` similarly hardcodes a `CHECK_NAMESPACE_TO_CAPABILITY` map from check-id namespace straight to a literal capability-id string — both admit this openly in their own comments. The registry (`apps/api/src/services/registry/registry.ts`) is only consulted afterward, to filter by `isEnabled`; the *set of possible capabilities per module* is still named in core. Clean fix: extract manifest-walking into `@webaudit/capability-sdk`; not done, six known capabilities don't yet force it. Worth a real decision — either accept this as a permanent, documented exception to non-negotiable #1, or do the extraction — rather than leaving the constitution's own "grounds for rejection on its own" language sitting unresolved against known code |
...
| 21 | `capability-upload.service.ts`'s conformance run hardcodes `controlLevel: 'NONE'` for every uploaded capability, and nothing in `runConformanceSuite` varies this or gates on the capability's own declared `requiredControlLevel` — found by the final production-readiness review's integration-seam pass (2026-09-04) | **Latent, not currently exploitable — needs a design decision, not a quick fix.** The real control gate (`apps/worker/src/module-runner/resolve.ts`'s `resolveApplicable`) is external to the capability and correctly refuses to even call a capability whose `requiredControlLevel` exceeds the scan's verified level — so today's actual production gate does not depend on conformance-checking having exercised every control level. But nothing in the capability contract forbids a future capability from internally branching on `input.controlLevel` (confirmed zero vendored capabilities do today), and conformance-checking would never exercise a branch gated above `NONE` if one existed — a false sense of "verified" for a capability that behaves differently, and dangerously, only at a higher control level. A second, independent reason this can't bite today: no uploaded capability can pass `manifest-valid` at all right now (Open Decision #20), so no upload reaches `passed: true` regardless. Real fix candidates: run conformance once per control level the manifest claims to need, or make it an enforced (not just documented) rule that capability code may never read `controlLevel` itself, only the runner may — same shape of decision as #20's own "don't synthesize a fix that makes a verification mean less." See `docs/superpowers/plans/2026-09-04-final-production-readiness-review.md`'s Finding E for the full trace. |
