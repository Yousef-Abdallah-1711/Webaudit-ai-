 ...
 * discipline exactly**: the guarded transition happens first; only a
 * genuine win enqueues phase 2; only *after* that does this write the
 * `DesignIntent` row. `DesignIntent.scanId` is `@unique`, and this route races
 * the worker's own deadline-handler for the same scan — whichever side's
 * guarded `updateMany` matches zero rows must do nothing further. Writing
 * `DesignIntent` speculatively before confirming the transition won would let
...
async function resume(
  ...
  // Only now, after the transition is confirmed won and phase 2 is enqueued —
  // matching questionnaire-timeout-handler.ts's own "inspect the outcome,
  // then write" ordering.
