...
export default tseslint.config(
  {
    ...
    ignores: [
      ...
      // T225's own compiled harness cache (gitignored, `.gitignore`'s "Build
      // output" section) — a flat, dependency-free `.js` esbuild output
      // regenerated by every sandbox-runner test/boot, not a file this repo
      // wrote or reviews. Found missing here during T236's whole-branch
      // `pnpm run lint` validation pass (Session 9, 2026-09-04): any local
      // run that had already exercised the sandbox once left this on disk,
      // and a bare `eslint .` reported a parse error against it (not in any
      // tsconfig, same class as the fixtures line above).
      'apps/sandbox-runner/.sandbox-build/**',
    ],
  },
