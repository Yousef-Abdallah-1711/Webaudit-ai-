 ...
 * produced by ../src/runner.ts against https://eink.ma/ — no mock
  ...
  return (
    <div>
      <PageHead
        ...
        title="eink.ma"
        meta={null}
        actions={<><a href={DL.report} download="eink-audit-report.md" style={{ textDecoration: 'none' }}><Button variant="secondary" size="sm">Full report.md</Button></a><a href={DL.audit} download="eink-audit.json" style={{ textDecoration: 'none' }}><Button size="sm">Raw JSON</Button></a></>}
      />
...
function exportFindings(map) {
  const lines = ['# Pentest — findings log', '', `Exported ${new Date().toISOString()}`, ''];
  const rows = [];
  ...
  return (
    <div>
      <PageHead
        ...
        title="Full pentest — storefront · admin · checkout"
        meta={`${RB.summary.phases} phases · ${total} test cases · ${done}/${total} worked · progress saved in this browser`}
        actions={
          <>
            <a href={'data:text/markdown;charset=utf-8,' + encodeURIComponent(findingsMd)} download="eink-findings-log.md" style={{ textDecoration: 'none' }}>
              <Button variant="secondary" size="sm">Export findings</Button>
            </a>
            <a href={DL.runbook} download="eink-PENTEST-RUNBOOK.md" style={{ textDecoration: 'none' }}>
      ...
      <Card padding={20} style={{ marginBottom: '16px', borderLeft: '3px solid var(--sev-high)' }}>
        ...
        <p style={{ font: 'var(--type-small)', color: 'var(--text-secondary)', margin: 0, maxWidth: '82ch', textWrap: 'pretty' }}>
          ...
          does <strong>not</strong> do — SQL/NoSQL injection, admin authentication and brute force, reaching the admin
          surface, IDOR on customer orders/checkout, SSRF, XSS, checkout price/coupon tampering, business logic. Each
          test case has an objective, steps, payloads, tools, evidence to capture, "secure
...
function App() {
  const [view, setView] = React.useState('readiness');
  const viewport = useViewportWidth();
  const compact = viewport < 920;
