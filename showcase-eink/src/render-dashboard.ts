    ...
    await readFile(join(VENDOR, f)).catch(() => {
      throw new Error(
        `dashboard/vendor/${f} missing. Fetch the three vendored libs once:\n` +
          `  curl -sSo showcase-eink/dashboard/vendor/react.js https://unpkg.com/react@18.3.1/umd/react.production.min.js\n` +
          `  curl -sSo showcase-eink/dashboard/vendor/react-dom.js https://unpkg.com/react-dom@18.3.1/umd/react-dom.production.min.js\n` +
          `  curl -sSo showcase-eink/dashboard/vendor/babel.min.js https://unpkg.com/@babel/standalone@7.26.4/babel.min.js`,
      );
    });
  }
...
<head>
  ...
  <title>WebAudit AI — eink.ma</title>
...
`;
  ...
  process.stdout.write(
    `  dashboard written: ${join(DASH, 'index.html')} (${kb} KB, fully self-contained)\n` +
      `  open it directly (file://) or run \`pnpm --filter showcase-eink serve\`\n`,
  );
}
