...
- [X] T153 [US2] Add `reverify` implementations to the six first-slice capabilities in `packages/capabilities-vendored/*/src/index.ts`. Each fetches the recorded URL once and re-runs exactly the one check its `checkId` names — never the others — returning PASSED / FAILED{evidence} / UNVERIFIABLE. `data-leak-scanner` is URL-only + kind-granular at re-check time (documented: never PASSED while any matching credential remains). `packages/capabilities-vendored/tests/unit/reverify.test.ts` — 16 tests.
